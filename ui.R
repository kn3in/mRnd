...
p("Assume that the causal effect of BMI on BP is \\(1.30 \\frac{mmHg}{SD}\\) and that the population regression coefficient of BMI on BP, including the effects of confounders, is \\(1.41 \\frac{mmHg}{SD}\\). Also assume that for the MR study we have a genetic instrument that explains \\(R^2_{xz} = 0.01\\) of variation in BMI (based on e.g. FTO SNP, which explains \\(\\sim 1 \\%\\) of the variation in BMI)\\(^{[2]}\\). Then we can calculate the power of an MR study using the following  parameters:"),
...
p("For an \\(\\alpha\\) of \\(0.05\\) and power of \\(0.8\\), the calculated minimum sample size for the Mendelian Randomization study is \\(N = 53,218\\). The reason why this sample size is so large is because BMI explains a small amount of variation in BP and because the genetic instrument explains a small proportion of variance in BMI."),
